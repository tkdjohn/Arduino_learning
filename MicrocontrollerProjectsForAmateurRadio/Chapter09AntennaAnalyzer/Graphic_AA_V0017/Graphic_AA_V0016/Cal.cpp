...
void Calibrate()
{
  char *bridgeReading[] = {" Vm ", " Vz ", " Vrz ", "Exit", " 25 Ohm", " 50 ohm", " 200 ohm", "Exit & Store"};
  ...
  tft.setTextSize(2);
  tft.setCursor(5, 0);
  tft.print("Select Reading to Zero");
  tft.setTextColor(ILI9488_GREEN, ILI9488_BLACK);

  for (int i = 0; i < 8; i++) {

    tft.setCursor(100, 40 + i * 28);
    tft.setTextColor(ILI9488_GREEN, ILI9488_BLACK);
    ...
    switch (indexCal) {
      case 0 :
        if (frequencyEncoderMovement != 0) {     //Change digit value
          ...
          tft.setCursor(250, 40 + indexCal * 28);
          tft.print(aveVm);
          tft.setCursor(400, 40 + indexCal * 28);
          tft.print(VmCal);
          frequencyEncoderMovement = 0;
        }
        if (digitalRead(MENUENCODERSWITCH) == LOW) {
        //Save  
          tft.setCursor(5, 40 + indexCal * 28);
          tft.print("Saved!");
        }
        break;
      case 1 :
